...
class RegenerateCertificationPdfs extends Command
{
    ...
    protected $description = 'Regenerate certificate PDFs for all approved certification submissions using the latest certificate template.';
    ...
    public function handle(CertificateGeneratorService $certificateGenerator): int
    {
        ...
        if ($this->option('dry-run')) {
            $this->info("{$total} approved certification PDF(s) would be regenerated.");
        ...
        $query->chunkById(100, function ($submissions) use ($certificateGenerator, &$processed) {
            foreach ($submissions as $submission) {
                ...
                $processed++;
                $this->line("Regenerated certificate PDF for {$submission->id}");
            }
        });

        $this->info("Regenerated {$processed} approved certification PDF(s).");
